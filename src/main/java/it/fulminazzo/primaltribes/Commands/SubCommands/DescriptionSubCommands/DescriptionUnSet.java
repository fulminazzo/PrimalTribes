...
public class DescriptionUnSet extends ClanSubCommand {

    public DescriptionUnSet(AbstractCommand command, String commandName) {
        super(command, "unset", Permissions.DESCRIPTION_UNSET, ClanPermission.DESCRIPTION_UNSET,
                StringUtils.getCommandSyntax(String.format("&c%s &aunset", commandName)),
                Message.HELP_DESCRIPTION_UNSET.getMessage(false, false),
    ...
    public void execute(CommandSender sender, Command cmd, String[] args) {
        checkClan(sender);
        clan.unsetDescription(sender);
        sender.sendMessage(Message.DESCRIPTION_UNSET.getMessage(!isAdminCommand(), isAdminCommand()));
    }
