...
import it.fulminazzo.primaltribes.Commands.SubCommands.MotdSubCommands.MotdSet;
import it.fulminazzo.primaltribes.Commands.SubCommands.MotdSubCommands.MotdUnSet;
...
public class Motd extends SuperClanSubCommand {

    public Motd(AbstractCommand command) {
        super(command, "motd", Permissions.MOTD, ClanPermission.MOTD,
                StringUtils.getCommandSyntax(String.format("&c%s &amotd", command.getName())),
                Message.HELP_MOTD.getMessage(false, false),
                !command.isAdminCommand(), CommandType.CLAN_ONLY);
        setSubCommands(Arrays.asList(
                new MotdSet(command, command.getName() + String.format(" &a%s", getName())),
                new MotdUnSet(command, command.getName() + String.format(" &a%s", getName()))
        ));
    }
