...
import it.fulminazzo.primaltribes.Commands.SubCommands.DiscordSubCommands.DiscordSet;
import it.fulminazzo.primaltribes.Commands.SubCommands.DiscordSubCommands.DiscordUnSet;
...
public class Discord extends SuperClanSubCommand {

    public Discord(AbstractCommand command) {
        super(command, "discord", Permissions.DISCORD, ClanPermission.DISCORD,
                StringUtils.getCommandSyntax(String.format("&c%s &adiscord", command.getName())),
                Message.HELP_DISCORD.getMessage(false, false),
                !command.isAdminCommand(), CommandType.CLAN_ONLY);
        setSubCommands(Arrays.asList(
                new DiscordSet(command, command.getName() + String.format(" &a%s", getName())),
                new DiscordUnSet(command, command.getName() + String.format(" &a%s", getName()))
        ));
    }
    ...
    public void execute(CommandSender sender, Command cmd, String[] args) {
            ...
            if (args.length > 0) {
                ...
                if (subCommand == null)
                    ...
                    sender.sendMessage(Message.USAGE.getMessage(!isAdminCommand(), isAdminCommand())
                        ...
                        discord != null && discord.replace(" ", "").equalsIgnoreCase(""))
        ...
        else if (discord.replace(" ", "").equalsIgnoreCase(""))
            sender.sendMessage(Message.LINK_NOT_SET.getMessage(!isAdminCommand(), isAdminCommand()));
        else sender.sendMessage(ChatColor.DARK_PURPLE + discord);
    }
